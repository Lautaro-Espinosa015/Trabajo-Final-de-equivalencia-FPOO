...
void actualizarJuego(float deltaTime) {
    nivel1.actualizar(deltaTime, prota, joypad, () -> {
        if (prota.getScore() >= 500 && nivel == 1 && HORDA == false) {
            nivel = 2;
...
void mousePressed() {
  if (estadoActual == ESTADO_MENU) {
    ...
    if (mouseX > width - 250 && mouseX < width - 210 && mouseY > 50 && mouseY < 90) {
      setVolumen(max(0, volumen - 0.1));
    }
    // Botón "+" (aumentar volumen)
    else if (mouseX > width - 190 && mouseX < width - 150 && mouseY > 50 && mouseY < 90) {
      setVolumen(min(1, volumen + 0.1));
    }
    // Botón Mute/Unmute
    else if (mouseX > width - 120 && mouseX < width - 30 && mouseY > 50 && mouseY < 90) {
      musicaActiva = !musicaActiva; // Alternar mute
      // Aquí puedes llamar a tu gestor de audio si lo tienes
      // GestorAudio.toggleMute();
    }
...
public void keyPressed() {
  if (estadoActual == ESTADO_JUGANDO || estadoActual == ESTADO_NIVEL2 || estadoActual == ESTADO_HORDA){
    ...
    if (key == 'e' && estadoActual == ESTADO_GAMEOVER) {
        reiniciarJuego();
        estadoActual = ESTADO_MENU;
    }
